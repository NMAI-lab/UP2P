...
public class Router extends Thread {
	 ...
	 */
	void addRelayMessageReceiver(MessageReceiver receiver) {
	 ...
	 * @return	The NodeConnection the query hit was received on
	 */
	public NodeConnection getQueryHitSource(GUID clientIdentifier) {
	 ...
	 * @return	The NodeConnection the query hit was received on
	 */
	public NodeConnection getPushSource(GUID clientIdentifier) {
	...
	void routePongMessage(RouteMessage m) {
		...
		if(connectionData.getPeerLookupEnabled()) {
			potentialHost = new Host(pongMessage, Host.HostState.ACTIVE);
		} else {
			potentialHost = new Host(pongMessage, Host.HostState.KNOWN);
		}
		
		// Ensure the local host isn't added as a new host
		if((potentialHost.getIPAddress().equals("127.0.0.1"))
				&& (potentialHost.getPort() == connectionData.getIncomingPort())) {
			LOG.info("Discarded host from PONG message (localhost): " + potentialHost.getIPAddress()
					+ ":" + potentialHost.getPort());
		} else {
			LOG.info("Added host from PONG message: " + potentialHost.getIPAddress()
					+ ":" + potentialHost.getPort());
			
			hostCache.addHost(potentialHost);
		}
	...
	void routePushMessage(RouteMessage m) {
		...
		NodeConnection originator = queryHitRouteTable.get(pushMessage.getTargetIdentifier());
	...
	void routeRelayMessage(RouteMessage m) {
		...
		NodeConnection originator = pushRouteTable.get(relayMessage.getTargetIdentifier());
