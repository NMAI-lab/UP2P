...
public class GNUTellaConnection {
	...
	private static short[] servantId; // static because unique for this peer anyway.. and provides easier access from anywhere
	 ...
	 */
	public static short[] getServantIdentifier() {
		return servantId;//Utilities.getClientIdentifier();
	}
	
	public static void setServantIdentifier( short[] id){
		servantId = id;
	}
	 ...
	 * @return
	 */
	public static String getServantIdentifierAsString() {
		String idstring = "";
		for (short k : getServantIdentifier()){
	...
	public String blacklistPeerFromIPPort(String IP, int port){
		List<NodeConnection> conns= this.getConnectionList();
		for(NodeConnection c : conns) {
			if(c.getConnectedServant().equals(IP) && c.getConnectedServantPort()==port){
				String guid=c.getRemoteServentId();
				hostCache.blacklistPeer(guid);
				return guid;
		}
		}
	...
	public void unBlacklistPeer(String guid){
		hostCache.unBlacklist(guid);
	}
	
	public boolean isBlacklisted(String remoteServentId) {		
		return hostCache.isBlacklisted(remoteServentId);
	}
